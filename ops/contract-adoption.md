...
committed. Each default branch was read for all fifteen configuration paths, the `package.json`
key, its protection, rules and rulesets, its workflows' text and its recursive tree. Every row
...
hold on the remote as well as in the record. None has its run as a required check on its default
branch. `cs-tournament`, public since 2026-09-24, could now carry one on the current plan but has
no workflow; the three private repositories, `cs-tracker`, `StreamVault` and `Mutuo`, still
cannot. Automation stays enabled nowhere.
...
|---|---|---|
| **The estate observation is one day's, by one token** | Every cell in the policy table was read on 2026-08-27 by `gh api` and says so, except the `list-wheel` row, re-read on 2026-09-13 by the same calls after Story 2-25 gave it a workflow. **Amended 2026-09-24:** re-read again that day for DW-90, and its CI and verdict cells describe `30e5e8b`, a commit ahead of its push, so they hold only once that push lands. A repository that gains a workflow or a configuration file tomorrow makes the table stale, not wrong; the method is recorded so it can be re-run. **Amended 2026-09-25:** the whole table was re-read that day by the same calls, the push had landed, and AD-22's refresh scope names the sweep from that day, with this row's trigger, so a re-read has a schedule; each row dates its own re-observation | **Decision.** Pending Operator action 5 |
