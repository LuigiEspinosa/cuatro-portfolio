...
| `list-wheel` | 4 `*.spec.ts` under `src/`, substantive component and service specs. Angular, Karma (`"test": "ng test --no-watch"`) | **no**: one workflow since 2026-09-13, `.github/workflows/deploy.yml` on `push` to `main` (Story 2-25, merged as `11f15cb`), whose two steps run the Anchor's Capacity Gate and a deploy over SSH and never the suite, so nothing runs `ng test` on a push (DW-90). No `continue-on-error`, no `\|\| true`. Was **no workflow** on 2026-08-27 | none: `404 Branch not protected`, no rules, no rulesets. **Observed 2026-08-27T22:40:10Z**; the same three answers **re-observed 2026-09-13T17:44:26Z** | none | off | off | 0 of 1 | **no** | **Re-observed 2026-09-13T17:44:26Z** by the same calls, one row and not the table, after Story 2-25's deploy workflow landed (Pending Operator action 5 stands): 48 blobs on `main`, the 43 of 2026-08-27 plus the five the story added (`Dockerfile`, `docker-compose.yml`, `.dockerignore`, `docker/Caddyfile`, the workflow), the four specs unchanged; all fifteen configuration paths `404` and no `renovate` key; security fixes `{"enabled":false,"paused":false}`; `allow_auto_merge=false`; one pull request in all, `#2`, authored and merged by hand at 17:30:56Z, so 0 of 1 where it was 0 of 0; `pushed_at` 2026-09-13T17:31:00Z. A workflow exists now and the verdict does not move: condition 3 asks for a run of the suite on every push, and this workflow runs none. Serves from `wheel.cuatro.dev` off the box since 2026-09-13, no longer from GitHub Pages. Public |
...
|---|---|---|
| **The estate observation is one day's, by one token** | Every cell in the policy table was read on 2026-08-27 by `gh api` and says so, except the `list-wheel` row, re-read on 2026-09-13 by the same calls after Story 2-25 gave it a workflow. A repository that gains a workflow or a configuration file tomorrow makes the table stale, not wrong; the method is recorded so it can be re-run | **Decision.** Pending Operator action 5 |
