...
| `list-wheel` | 4 `*.spec.ts` under `src/`, substantive component and service specs. Angular, Karma (`"test": "ng test --no-watch"`). From `30e5e8b` also `ops/deploy-remote.test.mjs`, the deploy script's and workflow's cases on `node:test`, which is tooling and counts for nothing under condition 2 | **yes, from the push of `30e5e8b`** (DW-90, closed 2026-09-24): `.github/workflows/deploy.yml` on `push` to `main` that changes more than Markdown (`paths-ignore: '**.md'`) and on `workflow_dispatch`, whose `test` job runs `npm ci`, `npm test` and `node --test ops/deploy-remote.test.mjs`, and whose deploy job needs it. No `continue-on-error`, no `\|\| true`. **Written from the commit, before its push**: at the **re-observation of 2026-09-24T19:59:07Z** the remote `main` was still `00f5957`, whose one workflow (Story 2-25, merged as `11f15cb`) ran the Anchor's Capacity Gate and a deploy over SSH and never the suite. Was **no workflow** on 2026-08-27 | none: `404 Branch not protected`, no rules, no rulesets. **Observed 2026-08-27T22:40:10Z**; the same three answers **re-observed 2026-09-13T17:44:26Z** and **2026-09-24T19:59:07Z** | none | off | off | 0 of 2 | **yes, from the push of `30e5e8b`**; **no** before it | **Re-observed 2026-09-24T19:59:07Z** by the same calls, one row and not the table, for DW-90 (Pending Operator action 5 stands): 48 blobs on `main` at `00f5957`, the 43 of 2026-08-27 plus the five Story 2-25 added, the four specs unchanged; all fifteen configuration paths `404` and no `renovate` key; security fixes `{"enabled":false,"paused":false}`; `vulnerability-alerts` `404`; `allow_auto_merge=false`; two pull requests in all, `#2` and `#3`, both authored and merged by hand on 2026-09-13, so 0 of 2 where it was 0 of 1 at the **re-observation of 2026-09-13T17:44:26Z**; `pushed_at` 2026-09-13T19:10:30Z. The verdict was **no** at every earlier observation, on 2026-09-13 because the workflow Story 2-25 added ran no suite, and moves to **yes** with the push. It is read as meeting condition 3 although a push that changes only Markdown runs nothing: the suite reads no Markdown, and a dependency change never touches Markdown alone. The fourth condition still fails, `main` being unprotected, so automation may not be enabled there. Serves from `wheel.cuatro.dev` off the box since 2026-09-13, no longer from GitHub Pages. Public |
