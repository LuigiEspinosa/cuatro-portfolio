...
| `cs-tracker` | Vendored `contracts/` verbatim as `assets/css/cuatro-contracts/`, nine files, and imports its `tailwind.css` from `assets/css/app.css` (Story 1-19, AD-15 route A) | `assets/css/cuatro-contracts/tokens.css` | `Contract v2.0.0` | 2.0.0 | **Observed 2026-09-24** by `git -C cs-tracker show 991d0f6:assets/css/cuatro-contracts/tokens.css`, line 2, the local re-vendor commit of Contract 2.0.0 (Operator ruling 2026-09-24), and by the probe's recorded-version case. **Local until the Operator pushes it** (Pending Operator action 8): `cs-tracker`'s remote `main` still reads `Contract v1.0.0` and the Elixir pin at `test/cs_tracker_web/token_contract_test.exs:342` still asserts it. It read `Contract v1.0.0` and 1.0.0, **observed 2026-08-27** by `git -C cs-tracker show 8adb8e2:assets/css/cuatro-contracts/tokens.css`, line 2, never restated from memory. The nine sha256 values of 1.0.0 are in `ops/cs-tracker-token-adoption.md:47-57`, and 2.0.0's three that moved beside them |
...
**On 2026-09-24 they read three different things, each observed.** Published `2.0.0`, from `40591cd`
on `dev`. Vendored `2.0.0` in `cs-tracker`'s local `991d0f6` and `1.0.0` on its remote: `gh api
repos/LuigiEspinosa/cs-tracker/contents/assets/css/cuatro-contracts/tokens.css?ref=main` read
`Contract v1.0.0` at 14:48:26Z with `main` at `32a466a`. Served `1.0.0`: `curl -sS -D - -A
cuatro-portfolio-dw15-check https://cuatro.dev/contracts/tokens.css` answered 200, `last-modified: Sun,
13 Sep 2026 19:26:55 GMT`, `Contract v1.0.0`, at 14:48:34Z. The served file moves when Epic 2 merges
to `main` (step 2) and the remote when the Operator pushes (Pending Operator action 8).
...
configuration, never a repository setting made on its own. **Decision.** **Amended 2026-09-25**,
by the re-observation of that day under § The estate, observed: the private repositories are
three, `cs-tracker`, `StreamVault` and `Mutuo`, since `cs-tournament` was published on 2026-09-24
(KV-2), and the required check still holds nowhere.
...
**Re-observed 2026-09-25 between 02:03:02Z and 02:06:01Z**, for AD-22's refresh scope, which names
this sweep from that day by Operator ruling 2026-09-24: the same calls as `LuigiEspinosa`, with the
same token scopes, one repository at a time, by a script under the session scratchpad that is not
committed. Each repository was read for `allow_auto_merge`, security fixes, alerts and every pull
request, and each default branch for all fifteen configuration paths, the `package.json` key, its
protection, rules and rulesets, its workflows' text and its recursive tree. Every row
below dates its re-observation in its Nature and method cell, with the old value and its date
wherever one moved. What moved: `cs-tournament` is public since 2026-09-24, so its protection
endpoints answer `404 Branch not protected` where they answered `403`; `list-wheel`'s push of
2026-09-24 landed; `cs-tracker`'s remote reached `32a466a` on 2026-08-27, after the first sweep;
...
| `cuatro-finance` | none: the default branch `main` holds one blob, `LICENSE` | no workflow | none: `404 Branch not protected`, no rules, no rulesets. **Observed 2026-08-27T22:39:08Z**, unchanged at the **re-observation of 2026-09-25T02:03:21Z** | none | off | off | 0 of 0 | **no** | A `dev` branch exists and was not inspected; the policy is about the default branch, where an automated merge would land. Public. **Re-observed 2026-09-25T02:03:21Z** by the same calls: nothing moved. The `dev` branch stays uninspected by Operator ruling 2026-09-24 (Pending Operator action 6) |
| `cuatro-tracker` | 118 test-shaped files: 105 under `__tests__/` across `app/`, `components/`, `lib/`, `store/` and the root, 12 `*.spec.ts` under `e2e/`, 1 under `tests/`. Vitest (`"test": "vitest --run --passWithNoTests"`) plus Playwright | **yes**: `ci.yml` on `push` to `main` and `dev`, `pull_request` to `main`; `test` job runs `pnpm test --coverage`, `e2e` job runs `pnpm test:e2e`. One `run: cat worker.log \|\| true` at `:212` prints a log and gates nothing | none: `404 Branch not protected`, no rules, no rulesets. **Observed 2026-08-27T22:39:19Z**, unchanged at the **re-observation of 2026-09-25T02:03:36Z** | none | off | off | 0 of 69 | **yes** | Public. **Re-observed 2026-09-25T02:03:36Z** by the same calls: nothing moved |
| `cs-tournament` | 119 test-shaped files: 57 `*.test.ts` under `lib/`, 25 `*_test.go` under `worker/`, 30 `supabase/tests/*_test.sql`, 2 support files under `test/`, and 5 `test_*.py` under `.claude/skills/*/scripts/tests/` which are bmad tooling and do not count. `"test": "vitest run"` | **no workflow**: no `.github/` in 662 blobs | none: `404 Branch not protected`, no rules, no rulesets. **Re-observed 2026-09-25T02:03:53Z**. Was not available at the **observation of 2026-08-27T22:39:30Z**, `403 Upgrade to GitHub Pro or make this repository public`, while the repository was private | none | off | off | 0 of 0 | **no** | A suite exists and nothing runs it on a push. **Public since 2026-09-24** (KV-2, Operator ruling 2026-09-24), private at the 2026-08-27 observation. **Re-observed 2026-09-25T02:03:53Z** by the same calls: the required-check cell moved with the visibility, and nothing else moved |
| `cs-tracker` | 68 test-shaped files on the remote `main` at `32a466a` (51 `*_test.exs`, 17 support and fixture files under `test/`), where it was 66 on 2026-08-27, before the push that brought Story 1-19's two files. ExUnit, `mix test`: **653 tests, 0 failures, 4 excluded**, **Observed 2026-08-27** on this host in 13.2 s against a Postgres 16 container | **no workflow**: no `.github/` in 187 blobs on the remote at `32a466a` (175 on 2026-08-27) and none in the local checkout | not available: `403 Upgrade to GitHub Pro or make this repository public` on protection, rules and rulesets. **Observed 2026-08-27T22:39:41Z**, unchanged at the **re-observation of 2026-09-25T02:04:09Z** | none | off | off | 0 of 0 | **no** | The case the definition exists for. Private. **Re-observed 2026-09-25T02:04:09Z** by the same calls: the test-shaped count moved from 66 to 68 and the blobs from 175 to 187 with the push of 2026-08-27T22:39:15Z, which reached `32a466a`; the two local commits since, `ae34619` and `991d0f6`, are not on the remote (Pending Operator action 8), and nothing else moved |
| `digital-library` | 34 test-shaped files, all under `__tests__/` beneath `apps/`. Vitest (`"test": "vitest run"`) | **yes**: `ci.yml` on `[push, pull_request]`, every push, `test` job runs `pnpm test -- --coverage`; `deploy.yml` on `push` to `main` also runs `pnpm test` before deploying | none required: `404 Branch not protected`; one active ruleset, `protect-main`, on `~DEFAULT_BRANCH` with the rules `deletion` and `non_fast_forward` and no required check. **Observed 2026-08-27T22:39:52Z**, unchanged at the **re-observation of 2026-09-25T02:04:25Z** | none | off | off | 0 of 21 | **yes** | Public. **Re-observed 2026-09-25T02:04:25Z** by the same calls: nothing moved |
| `list-wheel` | 5 test-shaped files on `main` at `50691bd`: 4 `*.spec.ts` under `src/`, substantive component and service specs, and `ops/deploy-remote.test.mjs`, the deploy script's and workflow's cases on `node:test`, which is tooling and counts for nothing under condition 2; `tsconfig.spec.json`, a compiler configuration, is not counted. Angular, Karma (`"test": "ng test --no-watch"`) | **yes**: `.github/workflows/deploy.yml` on `main` at `50691bd`, one commit past `30e5e8b` (DW-90), pushed 2026-09-24T20:35:12Z: on `push` to `main` that changes more than Markdown (`paths-ignore: '**.md'`) and on `workflow_dispatch`, its `test` job runs `npm ci`, `npm test` and `node --test ops/deploy-remote.test.mjs`, and its deploy job needs it; that push's run, 36055872795, passed. No `continue-on-error`, no `\|\| true`: the one match is a comment forbidding them (`deploy.yml:79`). Was **no workflow** on 2026-08-27, and on 2026-09-13 a workflow that ran no suite | none: `404 Branch not protected`, no rules, no rulesets. **Observed 2026-08-27T22:40:10Z**; the same three answers **re-observed 2026-09-13T17:44:26Z** and **2026-09-24T19:59:07Z**, and again **2026-09-25T02:04:42Z** | none | off | off | 0 of 2 | **yes**, since the push of 2026-09-24T20:35:12Z; **no** before it | **Re-observed 2026-09-24T19:59:07Z** by the same calls, one row and not the table, for DW-90 (Pending Operator action 5 stands): 48 blobs on `main` at `00f5957`, the 43 of 2026-08-27 plus the five Story 2-25 added, the four specs unchanged; all fifteen configuration paths `404` and no `renovate` key; security fixes `{"enabled":false,"paused":false}`; `vulnerability-alerts` `404`; `allow_auto_merge=false`; two pull requests in all, `#2` and `#3`, both authored and merged by hand on 2026-09-13, so 0 of 2 where it was 0 of 1 at the **re-observation of 2026-09-13T17:44:26Z**; `pushed_at` 2026-09-13T19:10:30Z. The verdict was **no** at every earlier observation, on 2026-09-13 because the workflow Story 2-25 added ran no suite, and moves to **yes** with the push. It is read as meeting condition 3 although a push that changes only Markdown runs nothing: the suite reads no Markdown, and a dependency change never touches Markdown alone. The fourth condition still fails, `main` being unprotected, so automation may not be enabled there. Serves from `wheel.cuatro.dev` off the box since 2026-09-13, no longer from GitHub Pages. Public. **Re-observed 2026-09-25T02:04:42Z** by the same calls: the push landed, so the CI and verdict cells describe the remote rather than a commit ahead of it; 51 blobs on `main` at `50691bd`, and 5 test-shaped files where the row read 4 specs, the fifth being `ops/deploy-remote.test.mjs`; all fifteen configuration paths `404`, no `renovate` key, security fixes off, `allow_auto_merge=false`, 0 of 2 bot-authored pull requests; nothing else moved |
| `StreamVault` | none: the default branch `main` holds one blob, `LICENSE` | no workflow | not available: `403 Upgrade to GitHub Pro or make this repository public` on protection, rules and rulesets. **Observed 2026-08-27T22:40:22Z**, unchanged at the **re-observation of 2026-09-25T02:04:58Z** | none | off | off | 0 of 0 | **no** | A `dev` branch exists and was not inspected. Private. **Re-observed 2026-09-25T02:04:58Z** by the same calls: nothing moved. The `dev` branch stays uninspected by Operator ruling 2026-09-24 (Pending Operator action 6) |
...
is therefore enabled nowhere and may be enabled nowhere yet. **Amended 2026-09-24:** `list-wheel`
joins the three with the push of its `30e5e8b` (DW-90), which makes four of eleven with a real test
suite and leaves three whose suite nothing runs on a push (`cs-tracker`, `cs-tournament`, `MaiCoin`).
...
| A key in the generated **Tailwind adapter** is **renamed** | MAJOR | Operator ruling 2026-09-24, `DESIGN.md` § Versioning, spine AD-16 | **Decision**, 2026-09-24. A `@theme` key is a published name: a consumer's `p-md` stops minting when `--spacing-md` goes. Contract 2.0.0 is the first such release, ledger event 1, and it closes the stated limit below that asked which category this was |
...
| **Rollout model** | deprecate, then migrate, then remove | `DESIGN.md:1042-1043`, `ops/token-contract.md:169` | **Decision.** There are no atomic commits across eight repositories, so a MAJOR ships the old name aliased to the new one first, every adopter migrates, and only then is the old name removed in a later MAJOR. **Waived once, 2026-09-24**, by Operator ruling, for Contract 2.0.0: an alias of an old spacing key would have published the collision the release removes, and the one adopter read none of the old names |
...
7. **Three steps and one column are still a guess written in the imperative**: publish, the Registry
   ...
   a scratch branch the same way before it is needed. *(2026-09-24: the first real release was a
   MAJOR, Contract 2.0.0, walked without a rehearsal: the Operator waived the alias step, and the
   re-vendor is one local commit in `cs-tracker` that no remote has seen, so it could be discarded as
   cheaply as a scratch branch. Steps 1, 4 and 5 were walked, 2 and the push are the Operator's, and
   step 6's window is recorded in ledger event 1.)*
...
|---|---|---|---|---|---|---|---|
| 1 | 2026-09-24 | 1.0.0 | 2.0.0 | `cs-tracker`, as one local commit on `main`, `991d0f6`, staging `assets/css/cuatro-contracts/` alone: three of the nine files moved (`tokens.css`, `tailwind.css`, `fonts.css`) and six are byte-identical. Not pushed, by the ruling: Pending Operator action 8 | **none.** A MAJOR with no deprecation window, by Operator ruling 2026-09-24: the eight Tailwind adapter keys `--spacing-2xs` to `--spacing-3xl` became `--spacing-s-*` at once, because an alias would have published the collision DW-15 records, and `cs-tracker` read none of them. The same release retargets `--ease-exit` to `cubic-bezier(0.33, 1, 0.68, 1)` (DW-103), a value change riding in it | `NO_COLOR=1 node ops/cs-tracker-adoption-probe.mjs` at 14:59:08Z, after `991d0f6` and the build of `ae7ffb2`: 19 cases, 18 PASS, 1 FAIL. The verbatim-copy case and the recorded-version case pass at 2.0.0; the one FAIL is the pipeline case, action 7 and DW-17, red since 2026-08-27 and unchanged. `mix test test/cs_tracker_web/token_contract_test.exs` in `cs-tracker`: 26 tests, 1 failure, the pin at `:342` still asserting `Contract v1.0.0`, which action 8 moves. Step 6: every Registry verification run on a branch carrying `ae7ffb2`, before the push, reads `Contract v1.0.0` on the remote against the declared 2.0.0 and fails naming both, the window the runbook describes; the daily run from `main` passes at 1.0.0 until Epic 2 merges | Bump `40591cd`; re-vendor `991d0f6` in `cs-tracker`; Registry edit `ae7ffb2` (`token_contract` 2.0.0, the Registry to 1.3.0). No work item was opened in `cs-tracker`'s own tracker: the ruling bounded the agent's act there to the one folder commit, so the adopter's remaining steps are action 8 here, addressed to the one person who works in both repositories |
...
|---|---|---|
| **The estate observation is one day's, by one token** | Every cell in the policy table was read on 2026-08-27 by `gh api` and says so, except the `list-wheel` row, re-read on 2026-09-13 by the same calls after Story 2-25 gave it a workflow. **Amended 2026-09-24:** re-read again that day for DW-90, and its CI and verdict cells describe `30e5e8b`, a commit ahead of its push, so they hold only once that push lands. A repository that gains a workflow or a configuration file tomorrow makes the table stale, not wrong; the method is recorded so it can be re-run. **Amended 2026-09-25:** the whole table was re-read that day by the same calls, the push had landed, and AD-22's refresh scope names the sweep from that day, with this row's trigger, so a re-read has a schedule; each row dates its own re-observation | **Decision.** Pending Operator action 5, closed 2026-09-25 into AD-22's scope |
...
| **A required status check holds nowhere, and cannot hold in four repositories** | The fourth condition for enabling automation is that the suite's run is a required check on the default branch. The Anchor's `main` protection names no check, `digital-library`'s ruleset requires none, five repositories are unprotected, and the four private ones answer `403` on protection, rules and rulesets on the current plan. So automation is enabled nowhere and, until a check is required somewhere, may be enabled nowhere. **Amended 2026-09-25**, re-observed: three private repositories answer `403` now, `cs-tournament` being public since 2026-09-24 and unprotected, six repositories in all; no check is required anywhere, so the conclusion stands. **Amended 2026-09-25:** the eight checks are required on `main` since 2026-09-25T08:22:04Z (`test`, `tokens-contract`, `fonts-contract`, `contract-purity`, `registry-schema`, `rendered-output`, `literal-conformance` and `lighthouse`, each pinned to the GitHub Actions app, `strict` false), read back through the API (DW-172), so the fourth condition now holds in this repository alone; no automation is enabled here | **Observed 2026-08-27 between 22:38:57Z and 22:41:09Z**, re-observed 2026-09-25 between 02:03:02Z and 02:06:01Z |
...
| **`cs-tracker`'s adoption is not on the remote** | The remote `main` was last pushed 2026-08-13 and carries no vendored folder. The adopted version was read off the local checkout at `8adb8e2`. **Amended 2026-09-25:** retired by action 1's push of 2026-08-27; the re-observation of 2026-09-25 read the remote `main` at `32a466a`, with `assets/css/cuatro-contracts/` present. The 2.0.0 re-vendor has a row of its own below | **Observed 2026-08-27.** Pending Operator action 1, closed 2026-08-27 |
| **Three repositories have a `dev` branch that was not inspected** | `cuatro-finance`, `StreamVault` and `poketracker-go` hold only `LICENSE` on `main`. The policy is about the default branch, where an unattended merge would land, and that is what was observed. **Amended 2026-09-25:** Operator ruling 2026-09-24 (Pending Operator action 6) keeps them uninspected: Dependabot reads its configuration only from the default branch, so a configuration on `dev` cannot turn automation on | **Decision** |
...
| ~~**The versioning table maps changes to `--token-*` names and values, and `contracts/tailwind.css` publishes generated `@theme` keys under the same version**~~ **Closed 2026-09-24.** | Struck rather than deleted. It read: the table does not say which category a change to one of those keys falls in; the first release to need the answer is the `--spacing-*` collision filed as DW-15; the category is that release's decision, and its ledger row names it. The Operator ruled on 2026-09-24 that renaming an adapter key is a MAJOR, the table above carries the row, and ledger event 1 names it | **Decision**, discharged by Contract 2.0.0 |
| **`cs-tracker`'s 2.0.0 is not on the remote** | The re-vendor is `991d0f6`, committed locally on 2026-09-24 and left unpushed by the ruling, behind the Operator's own unpushed `ae34619`. Until the push the adopted-versions row describes a checkout on one host, the deployed `cs-tracker.cuatro.dev` still serves 1.0.0, whose `max-w-md` DW-15 measured at 16px, and every Registry verification run on a branch carrying `ae7ffb2` fails on `cs-tracker`'s `token_contract` | **Observed 2026-09-24.** Pending Operator action 8 |
...
| 3 | **Decide the disposition of the 74 hit-target findings and the 38 transition findings** | Operator | They are Story 8.1's restyle under `RESTYLE-SPEC.md` § Family A (AD-20: a step carries nothing else), 18 of the 74 being daisyUI's defaults and 56 the application's own markup. The two causes behind the 38 transition findings are not geometry and may deserve an earlier decision: Tailwind v4's `transition-colors` transitions `outline-color` at 150 ms on every link that uses it, and the application's own `transition-all` on the three quick-link cards names `all`, so the ring's colour animates in on those controls; 29 more controls name `all` over `0s` and are excluded because they never animate. `EXPERIENCE.md`'s S-2 rule says never transitioned. The pass record names each | **2026-09-24.** Operator ruling 2026-09-24: all of it goes to Story 8.1, the 74 hit-target findings as restyle geometry and the 38 transition findings with them, and nothing is fixed in `cs-tracker` now. `epics.md` Story 8.1 carries a dated note naming both counts, this row and its mirror, `ops/cs-tracker-accessibility-pass.md` action 1, which closes with it. Action 4's re-run after Story 8.1 is the check |
| 4 | **Re-run `node ops/cs-tracker-accessibility-probe.mjs` after Story 8.1, after any Tailwind or daisyUI bump reaching `cs-tracker`, and on AD-22's refresh schedule**, and add this probe to that scope beside the two sibling probes | Operator | The probe needs a Postgres, a seeded database, a running `mix phx.server` and a Chromium, and the pass record says how to get all four in six commands. Nothing in CI can run it | **Scope half 2026-09-25**; the re-run half _not done_. Operator ruling 2026-09-24: AD-22's refresh scope names this probe and its record from 2026-09-25 (`2ec24c1`), beside the two adoption probes, with this row's two narrower triggers, after Story 8.1 and on any Tailwind or daisyUI bump reaching `cs-tracker`. The re-run waits on Story 8.1 or a bump; date this cell with it. Mirrored in `ops/cs-tracker-accessibility-pass.md` action 2 |
| 5 | **Re-gather the policy table on AD-22's schedule and whenever a repository gains a workflow or a dependency-automation configuration** | Operator | The script that gathered it is described in § The estate, observed, and takes about ninety seconds. A row that changes is a decision to record before the setting lands | **2026-09-25.** Operator ruling 2026-09-24: AD-22's refresh scope names this sweep from 2026-09-25 (`2ec24c1`), with this row's narrower trigger, a repository gaining a workflow or a dependency-automation configuration, so the schedule lives in the spine. Run once the same day, 02:03:02Z to 02:06:01Z, by the same calls: every row re-observed and dated in place, `cs-tournament`'s protection answers moved from `403` to `404` with its publication, `list-wheel`'s push observed, and no automation found anywhere (§ The estate, observed) |
| 6 | **Decide whether the three LICENSE-only repositories' `dev` branches should be observed too** | Operator | The policy is about the default branch. If work on `dev` is where an automated merge could ever be enabled, the table needs a second column | **2026-09-25.** Operator ruling 2026-09-24: no. Dependabot reads its configuration only from the default branch, so a configuration on `dev` cannot turn automation on, and the policy binds the default branch the table observes; `cuatro-finance` also moves into `apps/finance` in Story 3-5. The table keeps its one column, and the stated limit above carries the ruling |
| 7 | **Reconcile Story 1-19's pipeline pin with `cs-tracker`'s `32a466a`** | Operator | `32a466a` took `cuatro.fonts` out of `assets.setup` so the container build can run; `ops/cs-tracker-token-adoption.md` § Where the compiled output lands records the alias as a decision, `ops/cs-tracker-adoption-probe.mjs` pins it (its `The build pipeline places them` case is red at `32a466a`), and `cs-tracker/AGENTS.md:36-39` still says the task runs in `assets.setup`. Either the pin and the two records follow the commit, or the commit is revisited; this story changed none of them because none is its | **2026-09-24.** Operator ruling 2026-09-24: the pin and this repository's record follow `32a466a`. `The build pipeline places them` requires `cuatro.fonts` in `assets.build`, and in `assets.deploy` ahead of `phx.digest`, and no longer in `assets.setup`: commit `048793f`, with its unit case in `ops/__tests__/cs-tracker-adoption-probe.test.ts`. `ops/cs-tracker-token-adoption.md` § Where the compiled output lands carries a dated row, and the plain-shell re-run the same day exited 0 at 19 of 19. `cs-tracker`'s committed `AGENTS.md:36-39` still says the task runs in `assets.setup`; the Operator's uncommitted rewrite of that file carries no such line, and the file is the Operator's, so it was left to that commit. DW-17 closed. **Amended 2026-09-25:** `cs-tracker`'s own alias test now follows the commit too (`2519fe3`: `cuatro.fonts` refused in `assets.setup`, `setup` reaching it through `assets.build`); `cs-tracker/AGENTS.md:36-39` still says otherwise and carries the Operator's uncommitted edits, so it is left to him |
| 8 | **Push and deploy `cs-tracker` at `991d0f6`**, the local commit that re-vendored Contract 2.0.0 (Operator ruling 2026-09-24), which sits on `main` after the unpushed `ae34619` | Operator | The ruling limited the agent's commit to the vendored folder and forbade the push, so four steps are left, in `C:\CuatroEcosystem\cs-tracker-workspace\cs-tracker` on `main`. (1) Move the two things that still name 1.0.0, in one commit: `test/cs_tracker_web/token_contract_test.exs:342`, `assert tokens =~ "Contract v1.0.0"` to `"Contract v2.0.0"`, and the comment in `assets/css/app.css` that calls the contract `v1.0.0`; then `mix test` against a Postgres expects 0 failures (the token contract file read 26 tests, 1 failure, that pin, on 2026-09-24). (2) `git push origin main`, which carries `ae34619`, `991d0f6` and step 1's commit. (3) Deploy by the `docs/deployment.md` redeploy path, as action 1 of `ops/cs-tracker-token-adoption.md` did, then confirm on `cs-tracker.cuatro.dev` that the empty-state card DW-15 was found on (`dashboard_live.ex:252`, `max-w-md`) now wraps at 28rem rather than a word or two per line. (4) From this repository, `gh workflow run registry-verification.yml --ref dev`, and `--ref main` once Epic 2 merges, and confirm the `token_contract` line for `cs-tracker` passes at 2.0.0. Every run on a branch carrying `ae7ffb2` before step 2 fails naming 2.0.0 against `Contract v1.0.0`, which is the runbook's step 6 window and not a fault in the job. Date this row with the push, the deploy and the run URL | **2026-09-25.** Step 1: `02a7119` in `cs-tracker` moved the pin at `test/cs_tracker_web/token_contract_test.exs:342` to `Contract v2.0.0` and the `assets/css/app.css` comment; `mix test` against a Postgres 16 container read 653 tests, 1 failure, the stale alias pin action 7 names, fixed by `2519fe3` the same day (then 653, 0 failures on four of five runs; one run had one unnamed failure that did not recur). Step 2: pushed, `main` at `2519fe3` carrying `ae34619`, `991d0f6` and both. Step 3: `git pull && docker compose up -d --build` on the box, fingerprinted by the running release's digested `app-d5b1f6ed...css` reading `--ease-exit:cubic-bezier(.33,1,.68,1)`, the 2.0.0 value. Step 4: Registry verification run 36155337258 on `dev`, 34 of 34, `cs-tracker token_contract` passing at 2.0.0. The empty-state card DW-15 found was not viewed, being behind the Steam sign-in |
