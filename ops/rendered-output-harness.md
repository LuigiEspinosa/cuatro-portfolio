...
| The alias layer, per call site | `tests/e2e/anchor-aliases.pw.ts` | Does each of the twelve `--accent-dim` call sites resolve to the role its use earns, and do the two `--monument-bold` sites carry the weight a family alias cannot. Fifteen `--accent-dim` call sites until Story 2-9 deleted `ProjectCard.scss` with the component it styled; four `--monument-bold` sites until Story 2-14 deleted `ProjectsHero.scss` and three until Story 2-27 deleted `glitch-text.scss` on 2026-09-14. **Two `--accent-dim` call sites since 2026-09-23**, eight until Story 2-31 deleted `hud-label.scss` and rebuilt `WorkItem.scss` against the contract, and one `--monument-regular` site; the pseudo-element read and its planted control left with the last row that needed them. **One `--accent-dim` call site, and it an ornament, one `--monument-bold` site and no `--monument-regular` site since later on 2026-09-23**, when Story 2-30 deleted `error-page.scss`: the last boundary row left with it, and the `--monument-regular` clamp case with the last site it protected, the alias pinned at zero call sites instead. **No `--accent-dim` and no `--monument-bold` call site since later still on 2026-09-23**, when Story 2-33 rebuilt `WorkHero.scss`: both tables and their two per-site cases left with their last rows, and both aliases are pinned at zero call sites, so the file reads the layer itself (every alias to its role, the two literals, the base rule's ground and copy) | **Decision.** Story 1-18, counts amended 2026-09-06 by Story 2-9, 2026-09-14 by Story 2-27 and 2026-09-23 by Stories 2-31, 2-30 and 2-33 |
