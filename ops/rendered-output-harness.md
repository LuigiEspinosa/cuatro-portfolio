...
| The focus standard, A-1, on every interactive element on every route | `tests/e2e/accessibility-floor.pw.ts` | Does every visible tabbable on every route `app/` serves paint `--stroke-focus` solid `--token-focus` at `--focus-offset` when reached by Tab, with `:focus-visible` matched and no transition naming `outline` or `all` over a duration above zero; does it paint nothing under the pointer and nothing after a mouse click with navigation prevented; does the ring contrast at least 3:1 against the first painted ground beneath it, with each of the three token grounds read at least once and a control planted and labelled where no shipped element sits on one; and are `--token-focus`, `--token-accent-hover` and `--token-accent` pairwise different colours. Every threshold is resolved through a probe on the page, never typed | **Decision.** Story 2-26, **2026-09-13**. The rule is one global `:focus-visible` in `app/app.scss`, `RESTYLE-SPEC.md` § 4 verbatim, replacing nine per-component blocks; the readings, the grounds and the stated limits are in `ops/hub-accessibility-pass.md`. No screenshot is taken |
| DOM-order traversal and the skip control | `tests/e2e/accessibility-floor.pw.ts` | Does Tab from `body` visit exactly the visible tabbables in document order on every route, does the next Tab leave the document, does no `tabindex` compute above zero; and on the animated door, opened on a context that has not asked for reduced motion, is `.skip-control` a Tab stop that paints the ring | **Decision.** Story 2-26, **2026-09-13**. `EXPERIENCE.md:739` |
| The built CSS's z-index literals and depth tells | `tests/e2e/accessibility-floor.pw.ts` | Is every `z-index:<number>`, `box-shadow`, `text-shadow`, `linear-gradient(`, `radial-gradient(`, `repeating-linear-gradient(` and `conic-gradient(` occurrence in `.next/static/chunks/*.css` claimed by a dated ledger row, and is every row claimed back, tallied per value, property or function with the sum of the rows' counts held equal to what was observed; does every `var(--z-*)` name a layer `contracts/tokens.css` declares; and does an empty or absent build throw rather than pass over nothing | **Decision.** Story 2-26, **2026-09-13**. Read as text because `ScanlineOverlay.scss` writes a literal equal to a token by value, which a computed read passes. The ledger is `ops/hub-accessibility-pass.md` § The exemption ledger, the register entry KV-6 |
| The type floor, A-11 and A-12 | `tests/e2e/accessibility-floor.pw.ts` | Does every visible element carrying its own text, outside `aria-hidden` subtrees, compute at least `--t-3xs` on every route; every `<p>` at least `--t-2xs`, the four labels `DESIGN.md` places on `<p>` at the smallest step excepted by name; the six prose selectors at least `--t-sm`; nothing italic; no weight above the family's published range in `contracts/fonts.css` except what a ledger row claims, families the contract does not publish listed rather than judged; and does no `.scss` under `app/` or `components/` set `font-size` in `px` | **Decision.** Story 2-26, **2026-09-13**. `DESIGN.md:461-502`; the thresholds are resolved on the page |
