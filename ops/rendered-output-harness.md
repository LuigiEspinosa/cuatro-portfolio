...
| One level-1 heading per document, A-7 | `tests/e2e/accessibility-floor.pw.ts` | Does every route carry exactly one level-1 heading in its accessibility tree, read with `getByRole('heading', { level: 1 })` so the home route's `GlitchText` wrapper counts and its `aria-hidden` `<h1>` does not, and is the home route's named `Luigi Espinosa`; a route off the rule is a dated ledger row, and the 404, which renders its numeral and title as paragraphs, is one until Story 2-30 rebuilds it | **Decision.** Story 2-26's review pass, **2026-09-13**. Both route-level unit suites mock `GlitchText` away, so this is the only place the role lands on a real document |
