...
| The alias layer, per call site | `tests/e2e/anchor-aliases.pw.ts` | Does each of the twelve `--accent-dim` call sites resolve to the role its use earns, and do the four `--monument-bold` sites carry the weight a family alias cannot. Fifteen call sites until Story 2-9 deleted `ProjectCard.scss` with the component it styled; the `--monument-bold` sites are two since 2026-09-14, Story 2-14 having deleted `ProjectsHero.scss` and Story 2-27 `glitch-text.scss` | **Decision.** Story 1-18, count amended 2026-09-06 by Story 2-9 and 2026-09-14 by Story 2-27 |
