...
| Bytes on disk | **4,374** bytes over 3,714 characters and 92 lines | **Observed 2026-08-25**, by `Get-Item` and by reading the file as text. The two figures differ because the section rules and the header's middot are multi-byte UTF-8, not because the file carries a CR: a unit case asserts LF endings and one trailing newline |
...
| `--spacing-tap` | `--tap` | `p-tap` | **Decision.** The 44px hit-target floor, reachable as `min-w-tap` and `min-h-tap` as well as `p-tap` |
...
> Compile the adapter to an output file **inside the vendored `cuatro-contracts/` folder**, beside
> `fonts.css` and the `fonts/` directory. Serving the compiled stylesheet from anywhere else 404s
> every face, silently, and the page falls back to a system stack that looks almost right.

That is measured on every harness run, both ways. The check that loads the compiled file from inside
the vendored folder asserts each woff2 answers HTTP 200 and `document.fonts.check` reports each of
the three families available. The check beside it compiles the identical input one directory above
the vendored folder and asserts every face 404s, so the rule above is an observation rather than an
assumption, and cannot quietly stop being true.
...
Each refusal names the offending key and the file it came from, and each ends "Nothing was
published". That last clause is a fact rather than a hope: **observed 2026-08-25** against Style
Dictionary 5.5.2, `buildPlatform` formats every file in the platform before it writes any of them, so
a throw in either format leaves the output directory untouched. Verified by running the generator
with a corrupted map into an empty scratch directory and finding it still empty afterwards.
...
Each of the nine has a standing case in `packages/tokens/__tests__/tailwind-adapter.test.ts` that
runs the real generator against a corrupted copy of both inputs and asserts the output directory is
empty afterwards. They are permanent tests, not one-time probes, on the rule Story 1-11's review
established: a gate never observed to fail is not known to work, and a probe proves only the day it
ran.
...
four "writing" lines are printed before the build and say where a run would have written, which is
what makes a redirected run visible in a job log.
...
404 http://127.0.0.1:33563/fonts/geist-latin.woff2
404 http://127.0.0.1:33563/fonts/bricolage-grotesque-latin.woff2
404 http://127.0.0.1:33563/fonts/geist-mono-latin.woff2
```

`document.fonts.check` reported all three families unavailable. This is what makes the placement rule
...
| 2 | **Re-check the Tailwind pin on the settled-inputs schedule** (AD-22) | Operator | `tailwindcss` and `@tailwindcss/cli` are pinned exact at 4.3.3. The pinned set of stock utility names in `tests/e2e/contract-tailwind.pw.ts` is a fact about that version, and a bump will move it. Bump both packages together and read the negative control's output | _not done_ |
...
| 5 | **Run `/bmad-project-context` to refresh the `bmad:context` block in `AGENTS.md`** | Operator | Still open from Stories 1-10 and 1-12, and this story widens it again: `AGENTS.md:55-57` says Playwright is not installed and that no acceptance criterion may claim a browser check, which is now false for three spec files, and `AGENTS.md:52-53` still describes CI as typecheck and tests only against a file with four jobs | _not done_ |
