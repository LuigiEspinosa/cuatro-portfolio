...
**Re-read 2026-09-26 by the registry-1-5-0 package**, after Registry 1.5.0 added two `Live` rows to
the Directory on `/` (Operator ruling 2026-09-25). The same method: in
`mcr.microsoft.com/playwright:v1.62.1-noble`, `@lhci/cli` 0.15.1 driving Lighthouse 12.6.1 under the
image's Chrome for Testing 151.0.7922.34, three runs per URL, `lhci collect` then `lhci assert`
against `.lighthouserc.js`, the build taken with the Umami variables empty and served by `pnpm start`
on port 3000, nothing uploaded. Build `osSe2QxbtidtNJjkm9IKh` on the package's working tree over
`ebe9a39`. `lhci assert` reported every assertion green. Accessibility, best practices and SEO read
1.00 on all nine runs, with no failing audit in the three gated categories; performance, not gated,
read 0.66, 0.66 and 0.65 on `/`, 0.92, 0.91 and 0.92 on `/work`, and 0.91, 0.87 and 0.87 on `/cv`.
The eight rows the Directory now draws leave the accessibility score where it was.
