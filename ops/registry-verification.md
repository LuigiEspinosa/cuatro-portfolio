...
| Alert path last verified | **not yet.** The first `push` run on `dev`, before the secret exists, is the deliberate failure that proves the mail path: it exits 2 naming `REGISTRY_VERIFICATION_TOKEN`, and the Operator confirms the mail arrived. Pending Operator action 3 dates this cell | **Decision.** `ops/monitoring.md` § Re-testing the alert path: a test that was not recorded did not happen |
...
**No run has been observed.** The workflow, the script, this record and the suite exist on `dev` and
have not been pushed; pushing is the Operator's act. When the first two runs have happened this
section records, for each: the run URL, the trigger, the exit code, the "Set up job" lines naming the
runner image and machine name (the AD-18 confirmation that the job ran on a GitHub-hosted runner), and
the check count. The first run, before the secret, is expected to exit 2 with one line naming
`REGISTRY_VERIFICATION_TOKEN`; the second, after it, to exit 0 with 35 checks passed and the table in
its job summary.
