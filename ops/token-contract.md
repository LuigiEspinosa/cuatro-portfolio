...
| ~~`contracts/tailwind.css`, the `@theme inline` adapter~~ **Published 2026-08-25 by Story 1-13** | Generated from this file, and it needed `fonts.css` to exist first. `packages/tokens/build.mjs` now emits it as a second file of the same Style Dictionary platform, driven by `packages/tokens/theme-map.json`, so one `pnpm tokens:build` writes both and the `tokens-contract` drift gate below already covers it with no new job. The full record is `ops/tailwind-adapter.md` | **Observed 2026-08-25**, by reading the published file and running the drift gate against it. Owner: Story 1.13, closed |
