...
**SM-1 by front door**, added 2026-09-24 by Operator ruling (DW-88). The reached visits of the query
above, split by the `door` each `suite-reach` carries. It splits the numerator only: a visit that
never reaches sends no event, so its door is recorded nowhere (§ Stated limits). An event sent
before the door reached production carries no `door` row and reads `unrecorded`, which is why the
join is a left join.

```
/* SM-1 by front door: the reached visits above, split by the door each came through. */
with hub as (
  select distinct visit_id from website_event
  where event_type = 1 and split_part(url_path, '#', 1) = '/'
    and created_at >= '2026-09-01' and created_at < '2026-10-01'
)
select coalesce(d.string_value, 'unrecorded') as door, count(distinct e.visit_id) as reached
from website_event e join hub using (visit_id)
  left join event_data d on d.website_event_id = e.event_id and d.data_key = 'door'
where e.event_type = 2 and e.event_name = 'suite-reach'
  and e.created_at >= '2026-09-01' and e.created_at < '2026-10-01'
...
| **Reach is not split by front door** | The event carries no data, so SM-1 cannot be read per door, which is the diagnosis SM-1 exists for. Reading the door means coupling to `HomeLayout`'s flat modifier, which Story 2-29 rewrites. **Split 2026-09-24, and what remains is the denominator.** The event carries `door` since the Operator's ruling of that day (DW-88) and § How each metric is read joins it, so the reached visits read per door. The Hub sessions do not: a page view carries no door, and a visit that never reaches sends nothing, so no reading knows how many visits each door had. A per-door share of sessions would need the door on the page view, a change to the tracker's page views that the ruling's no-new-event line leaves out. A visit that reached in two tabs on different doors counts once under each | **Decision.** Filed as deferred work; the seam is one `data` argument. **Closed 2026-09-24** for the numerator, DW-88; the denominator is stated here |
...
| The front door on the reach event, DW-88: sent as the one key `door`, `flat` or `narrative`, handed down by `HomeLayout` from its decided path, and nothing polled or observed while the path is undecided; the Directory mounting no reach of its own; the record's Data cell and the SM-1 by-door query held to the key the component sends | `SuiteReach.test.tsx`, `HomeLayout.test.tsx`, `SuiteDirectory.test.tsx`, `ops/__tests__/visitor-instrumentation.test.ts`; `tests/e2e/visitor-instrumentation.pw.ts` reads `{ door: 'narrative' }` on the default door and `{ door: 'flat' }` on the reduced-motion door | Nine unit cases red on the tree before the change, each for its reason, then green. In the pinned image, four of the e2e cases that read a sent event, two per door, failed on the package's baseline with the data `undefined`; every such case passes on its final tree | **Observed 2026-09-24**, Operator ruling of that day |
...
| 6 | **Run the SM-1 by-door query once** after the Epic 2 merge to `main` has deployed: from a browser profile without the `umami.disabled` flag (action 4), open https://cuatro.dev/ and scroll to the Suite, then over `wsl -d Ubuntu-22.04 ssh deploy@177.7.52.248` run the query under § How each metric is read's SM-1 by front door in `docker exec -it cuatro-portfolio-anchor-db-1 psql -U umami -d umami`, with the month set to the current one, and fill this cell with the date | Operator | DW-88, added 2026-09-24. The query was written against the schema confirmed in action 1 and held to the component's key by `ops/__tests__/visitor-instrumentation.test.ts`, but it has not run on the box, because the door reaches production only with the merge. The pass is a row whose `door` is `narrative` or `flat` beside any `unrecorded` ones; a query error, or `unrecorded` alone after the visit, is a finding for this file | _not done_ |
