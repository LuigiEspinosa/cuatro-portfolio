...
| KV-7 | Two `Live` Registry members serve from Vercel, outside the Cloudflare proxy | AD-26, AD-17b | **Open**, tolerated deliberately | 2026-09-26 | unassigned | _not retired_ |
...
| Status | **Open**, tolerated deliberately | **Decision** |
...
| Retired by | **`unassigned`** | **Decision.** It retires when both hostnames are proxied through Cloudflare with Full (strict) and covered by the zone's bot rules, or when both leave `Live`. No story plans either, so it is a standing question for the Operator, carried as Pending Operator action 10 below |
| Retired on | _not retired_ | Filled when neither hostname is DNS-only, verified by `nslookup` answering Cloudflare addresses and a `cf-ray` on the response, or when neither entry is `Live` |
