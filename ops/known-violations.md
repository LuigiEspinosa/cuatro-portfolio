...
| KV-6 | Seven z-index literals, eight depth tells, six clipped rings, two synthesised weights and one route with no level-1 heading ship on the Hub | UX-DR44, UX-DR45 (`epics.md:665-676`), A-1 and A-7 (`EXPERIENCE.md:760,766`), the hallmark floor `epics.md:3053-3061` | **Open**, tolerated deliberately | 2026-09-13 | Stories 2-28, 2-29, 2-30, 2-31, 2-32 and 2-33 | _not retired_ |
...
## KV-6: Seven z-index literals, eight depth tells, six clipped rings, two synthesised weights and one route with no level-1 heading ship on the Hub
...
sum spelled out. Seven `z-index` literals (`20`, `5`, `3`, `10`, and `2` three times); eight
depth tells (`linear-gradient(` six, `radial-gradient(` once, `repeating-linear-gradient(` once;
**fifteen until 2026-09-14**, when Story 2-27 deleted `glitch-text.scss` and the seven
`text-shadow` steps of its loop with it, the first of the seven rebuilds to land; the entry read
seventeen from 2026-09-13 until the review pass of the same day, an arithmetic slip the suite now
makes impossible, and the "What is in breach" cell below went on reading seventeen against a
heading that said fifteen until Story 2-27 corrected it); six Tab stops whose ring is
clipped within its reach (the two home nav links and the three contact links by their panels'
`clip-path`, the skip-link by the viewport's edge); two elements whose weight the browser
...
| What is in breach | **Seven z-index literals** in four files: `HomeLayout.scss` `20`, `5` and `3`; `ScanlineOverlay.scss` `10`, which equals `--z-raised` by value and resolves to nothing; `WorkHero.scss` `2` twice; `error-page.scss` `2`. **Eight depth tells** in four files: the cybercore grid as two `linear-gradient` images on each of `app/app.scss`'s `body#work`, `HomeLayout.scss`'s `body[id='']` and `error-page.scss`'s `.error-page`; `ScanlineOverlay.scss`'s `radial-gradient` vignette and `repeating-linear-gradient` scanlines. Fifteen in five files until 2026-09-14: the seven `text-shadow` steps of `glitch-text.scss`'s glitch loop left with Story 2-27 (this cell read "seventeen" from 2026-09-13 until that story, the slip the heading's paragraph records). **Six clipped rings**, all on `/`: `.home-panel--nav` and `.home-panel--contact` carry a `clip-path: polygon(...)` bounded by their own border box and no padding (`HomeLayout.scss:62`, `:69`), and the two `a.nav-link` and three `.contact-container a` sit flush with the panel's edges, so a 2px ring at 3px offset paints on one or two sides only; the skip-link is parked at `inset-block-start: 0; inset-inline-start: 0` (`SkipLink.scss:33-34`), so when it is revealed its top and left ring sit past the viewport's edge. **Two synthesised weights**: `WorkItem.scss` asks Geist Mono for `600` on `.work-item__initiative` and the face publishes `400`, on `/work` and on `/cv`. **One route with no level-1 heading**: the 404 renders its numeral and its title as `<p>` (`Error404.tsx:62-68`), so its accessibility tree carries no heading at all | **Observed 2026-09-13** in `mcr.microsoft.com/playwright:v1.62.1-noble`, by reading the built stylesheets as text, every visible text's computed `font-weight` against `contracts/fonts.css`, every Tab stop's box against its clipping ancestors and the document's edge within the ring's reach, and `getByRole('heading', { level: 1 })` per route. Every occurrence is a row in `ops/hub-accessibility-pass.md` § The exemption ledger |
...
| Where the repairs are booked | **By ownership of the file, not by a criterion naming the tell.** Story 2-27 rebuilt `GlitchText` (`glitch-text.scss`, deleted with its row on 2026-09-14), Story 2-28 `ScanlineOverlay` (`ScanlineOverlay.scss`), Story 2-29 `HomeLayout` (`HomeLayout.scss`), Story 2-30 `Error404` (`error-page.scss`), Story 2-31 `WorkItem` (`WorkItem.scss`), Story 2-33 `WorkHero` (`WorkHero.scss`) and, by ownership of the `/work` surface, `app/app.scss`'s `body#work` block. The clipped home rings are Story 2-29's with the panels that clip them; the 404's missing heading is Story 2-30's with the surface. **The skip-link's clipped ring is booked to Story 2-32 by ownership of the top-of-page chrome alone**, the KV-4 precedent for a row booked by ownership and not by a criterion: `SkipLink` was built by Story 2-13, which is `done`, no redesign story names it, and Story 2-32 is the one that rebuilds the header, the logo and the contact links that sit at the top of every surface. **Not established**: no acceptance criterion in `epics.md` names a `z-index`, a shadow, a gradient, a clipped ring or the 404's heading for any of the seven; each is booked because its story rebuilds the file token-native and a token-native rebuild has no literal to keep | **Observed 2026-09-13**, by reading `epics.md:3069` onwards. The skip-link booking is flagged in Story 2-26's completion report for the Operator to confirm or reassign |
...
| Retired by | **Stories 2-28, 2-29, 2-30, 2-31, 2-32 and 2-33**, each deleting its rows in the commit that rebuilds its file. Story 2-27 was the seventh and went first, on 2026-09-14, deleting `shadow-glitch-loop` with `glitch-text.scss` | **Decision.** The sweep makes that unavoidable: a row whose tell the built CSS no longer carries fails as stale, so a story cannot repair a file and leave the exemption behind. Story 2-9, Story 2-15 and Story 2-17 are KV-4's demonstrations that the mechanism works; this entry copies it, and Story 2-27 is its first |
