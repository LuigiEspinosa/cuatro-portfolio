...
/**
 * The characters of `text`, one per grapheme cluster where the runtime can segment (every Node
 * the server runs, Chromium, Firefox 125 and Safari 14.1 onwards) and one per code point where it
 * cannot. The server always has `Intl.Segmenter`, so the served markup is always per grapheme; the
 * fallback splits identically on any text without a combining sequence or a flag, and the text
 * `HomeLayout.tsx:110` passes is ASCII, so a browser without it hydrates the same fourteen spans
 * rather than throwing inside the client boundary and taking the route to the error page.
 */
 ...
 *
 * `tsconfig.json`'s `lib: esnext` types `Intl.Segmenter`; the inline custom properties need the
 * `CSSProperties` cast because `@types/react` declares no `--*` index signature.
 */
