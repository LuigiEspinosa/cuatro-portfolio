...
describe('GlitchText', () => {
  ...
  it('renders the text as the text content of a real h1, and nothing else', () => {
  ...
  it('is the level-1 heading named by its own text, with no ARIA attribute anywhere in it', () => {
    const { container } = render(<GlitchText text='Luigi Espinosa' />);
    const heading = screen.getByRole('heading', { level: 1 });
    expect(heading.tagName).toBe('H1');
    expect(heading.textContent).toBe('Luigi Espinosa');
    expect(screen.getAllByRole('heading')).toHaveLength(1);
    expect(container.querySelectorAll('[aria-label], [aria-hidden], [role], [aria-level]')).toHaveLength(0);
  });
  ...
  it('carries no delay as 0s, so the stylesheet never reads an undeclared property', () => {
