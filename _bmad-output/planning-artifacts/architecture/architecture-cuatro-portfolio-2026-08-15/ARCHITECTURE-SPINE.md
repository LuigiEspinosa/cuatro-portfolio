...
- **Amended 2026-09-24**, by Operator ruling. The rule counted "three live hostnames" diverging from their ids. The count is dropped rather than updated, because five of the six `Live` entries diverged by then (only `cs-tracker` sits at `<id>.cuatro.dev`), and a count restated in prose goes stale with the next hostname.
...
- **Rule:** Required on every entry: `id`, `name`, `description`, `status`, `tech[]`, `source`, `demo`, `identity`. Optional: `live`, `family`, `absorbed_into`, `token_contract`. `demo` and `identity` are **required with an explicit value including `none`**: an absent field is never a permitted way to say "not applicable". `live` is required when `status` is `Live` and forbidden when `status` is `Archived`. `status` accepts exactly `Live`, `Complete`, `In progress`, `Archived`. The envelope carries `contract_version`: a wording-only edit to a `description` or a `name` is a patch bump, any other value change is a minor bump, and a field renamed or removed is a major bump.
- **Split 2026-09-24**, by Operator ruling, answering `ops/registry-schema.md` Pending Operator action 6. The rule read "a value change is a minor bump, any field rename is major", which made every editorial correction a minor bump, so `contract_version` sat at `1.1.0` through Story 2-6's nine rewritten descriptions and Story 2-25's `list-wheel` `live` and `tech`. The patch level lets a consumer tell an editorial pass from a change in the data. The Registry moved to `1.2.0` the same day, one minor covering every value change since `1.1.0`, and `contracts/registry.schema.json` states the same rule at `contract_version`.
...
- **Rule:** The Registry's unit is the **application**, not the repository. An application that has been archived or absorbed keeps its entry, with `absorbed_into` naming the application it has been, or is set to be, folded into. Repository count and entry count are different numbers by design and neither validates the other.
- **Widened 2026-09-24**, by Operator ruling. The rule read "with `absorbed_into` naming where its code now lives", and neither fold had happened: `tcg-tracker` and `connect-four-react` both describe theirs as intent. A recorded intent now counts as well as a completed fold, so the field alone does not say the code has moved, and `source` says where it sits today. No Registry value changed; `contracts/registry.schema.json` and the gate's two `absorbed_into` refusals say the same.
...
- **Amended 2026-09-24**, by Operator ruling (DW-15). A key in the generated `tailwind.css` is a published name, so renaming one is a rename under this rule and major. Contract 2.0.0 renamed the adapter's eight spacing keys to `--spacing-s-*`, because each shadowed Tailwind's container key of the same size, and **waived deprecate → migrate → remove for that release only**: an alias would have published the collision the release removes, and the one adopter, `cs-tracker`, read none of the old names.
...
- **Rule:** An epic whose first story opens after **2026-11-15** records a refresh check before that story opens. The scope of the check is fixed: Traefik, PostgreSQL, restic and `docker-rollout` versions; Clerk and Railway pricing; the Style Dictionary ≥5.5.1 security floor; the Let's Encrypt lifetime schedule (90 → 64 days in Feb 2027 → 45 days in Feb 2028); and, **added 2026-08-16, the observed serving topology**, meaning which addresses the estate's hostnames resolve to, what terminates TLS on each, and whether each DNS record is proxied or DNS-only; and, **added 2026-09-23, the two `cs-tracker` adoption probes**, meaning `ops/daisyui-route-probe.mjs` with its record `ops/daisyui-route.md` and `ops/cs-tracker-adoption-probe.mjs` with its record `ops/cs-tracker-token-adoption.md`, each re-run and its exit code written into its record, and each also re-run between checks on the two narrower triggers the records name: any Tailwind or daisyUI bump reaching `cs-tracker`, and any contract MINOR; and, **added 2026-09-24, the scheduled Registry verification**, meaning `.github/workflows/registry-verification.yml` dispatched by `workflow_dispatch` and confirmed green, which re-enables a schedule GitHub disabled after 60 days without a push to any branch (DW-85, whose heartbeat monitor the UptimeRobot free plan refused); and, **added 2026-09-25 by Operator ruling 2026-09-24, four re-checks the ops records asked for with no schedule to name**: the `cs-tracker` accessibility probe, `ops/cs-tracker-accessibility-probe.mjs` with its record `ops/cs-tracker-accessibility-pass.md`, re-run and its exit code written into that record, and re-run between checks after Story 8.1 and on any Tailwind or daisyUI bump reaching `cs-tracker`; the estate automation-policy sweep of `ops/contract-adoption.md` § The estate, observed, its cells replaced by that file's maintenance rule, and re-run between checks whenever a repository gains a workflow or a dependency-automation configuration; the upstream font pins in `packages/fonts/sources.json`, each pinned source compared with its upstream and any change to the published latin subsets recorded in `ops/font-contract.md`, and re-checked between checks before any regeneration of the published faces; and the Tailwind pin, `tailwindcss` and `@tailwindcss/cli` in `package.json`, compared with the latest release, with `TAILWIND_NAMESPACES` in `packages/tokens/build.mjs` diffed against the release notes of any version the pin moves to, and re-checked between checks on any Tailwind bump. Nothing outside that list re-opens, and a decision is never re-litigated because time passed.
...
- **The deprecated Origin CA Key is a different thing, and this decision does not use it.** Cloudflare deprecated the legacy **Origin CA Key**, an API credential for issuing origin certificates programmatically, in favour of account or user API tokens. **Origin CA certificates themselves are current and supported.** This decision issues one certificate by hand from the dashboard, so it touches neither the deprecated key nor any token. Recorded because the naming collision invites a future reader to conclude that AD-26 rests on a deprecated feature. If issuance is ever automated, it uses an API token.
...
| Dates & versions | ISO 8601, UTC. Semver on every contract: value change minor, rename major. The Registry also takes a patch, for a wording-only edit to a `description` or a `name` (AD-5, split 2026-09-24). |
...
- **Hostnames for `cs-tournament` and `list-wheel`**: `wheel.cuatro.dev` is a placeholder in the UX mocks (O-5); the Registry is the only mapping (AD-3) and it is written when the hostname is chosen. **Closed 2026-09-02 by Story 2-4** (recorded here 2026-09-24 by Operator ruling): `cs-tournament` is `Live` at `inclusivcup.vercel.app` and `list-wheel` takes `wheel.cuatro.dev`, where Story 2-25 placed it on 2026-09-13; the Registry carries both. **Amended 2026-09-25 by Operator ruling 2026-09-24:** Vercel is removed from the estate, so `cs-tournament` has no hostname now: it is `Complete` in Registry 1.4.0 with no `live` value, and Story 3.7 declares one when it places the application on the box.
