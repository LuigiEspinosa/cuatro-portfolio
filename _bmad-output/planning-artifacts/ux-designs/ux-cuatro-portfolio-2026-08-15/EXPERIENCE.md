...
| ~~O-10~~ | ~~The palette reconciliation.~~ **Decided 2026-08-15: the contract palette wins.** Cybercore's hardcoded values map to token roles; full mapping in [`rebaseline-2026-08-15.md`](rebaseline-2026-08-15.md) § O-10. Story 1.18 is unblocked | — |
| **O-12** | **Three expressive surfaces have no role in a single-hue palette.** GlitchText's red/cyan aberration pair (`glitch-text.scss:33`–`:68`) needs opposing hues the palette cannot supply; ScanlineOverlay's `rgba(0,0,0,…)` (`:6`, `:16`, `:17`) violates "nothing is pure"; the decorative numeral at `error-page.scss:28` maps to a role marked *ornament only, never text*. Each needs a named exception or a redesign — **not a dev-story decision** | **Migration steps 3–4** (Stories 2.18–2.19) |
