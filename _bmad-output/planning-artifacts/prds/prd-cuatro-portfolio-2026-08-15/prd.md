...
| `cs-tournament` | Merge into the Anchor + migrate off external PaaS | ~~`[ASSUMPTION: Live on Vercel]`~~ `Complete`, deployed nowhere: see the amendment below the table | Rendered |
...
**Amended 2026-09-25 by Operator ruling 2026-09-24.** The `cs-tournament` row read `[ASSUMPTION: Live on Vercel]`. Story 2-4 confirmed it `Live` at `inclusivcup.vercel.app` on 2026-09-02 (§13 Q9), and the ruling removed Vercel from the estate, so its Status today is `Complete`: finished, and deployed nowhere until Epic 3 merges it into the Anchor and places it on the box. That removal is the migration off external PaaS. `Complete` renders (FR-35), so the six below stand. Only this row is amended here; `ops/estate.md` carries every application's confirmed Status.
...
- **NFR-4: Cost-bound.** All-in spend stays within $40–100/month. The VPS is prepaid, so only marginal spend counts against this. **Confirmed 2026-08-16:** the prepaid box is the **Hostinger KVM 2**, at $0 marginal per month, with the term running to **2028-07-19**. "Prepaid to 2028" was accurate. The second address running on that date is retired by Story 1.21, and its cost until then is not recorded anywhere. **Nothing in the plan watches 2028-07-19.** It is a fixed deadline rather than a stale input, so AD-22's refresh check is the wrong instrument for it; it needs a reminder, not a re-verification.
...
**Cost.** $40–100/month all-in. The Hostinger KVM 2 is prepaid to **2028-07-19** at $0 marginal per month (confirmed 2026-08-16), so the live question is marginal spend: managed identity, external monitoring, and any overflow hosting. Overflow hosting for two heavy applications is budgeted at $15–30/month and stays inside the ceiling.
...
- **SM-7: Estate size.** Repository count under Ecosystem governance. Target 13 at MVP, 10 at end state. Validates §5. *Amended 2026-09-26 by Operator ruling (DW-249): the targets were 12 and 8. Listing `covidmap` and `future-vizion` in Registry 1.5.0 (Operator ruling 2026-09-25) added two public repositories the Operator keeps permanently, so both targets rise by the same two rather than the estate missing its own measure.*
...
7. ~~**Does the Hub's narrative survive the reduced-motion path intact?** FR-2 requires the Suite Directory to be reachable regardless; whether the narrative gets a static fallback or is simply skipped is undecided.~~ **Closed 2026-08-15** in `EXPERIENCE.md` § The homepage: two paths, one destination: one non-3D front door, a typographic hero, and no static poster of the scene. Story 2-13 shipped it on 2026-09-07. *(Struck 2026-09-24 by Operator ruling, DW-45.)*
...
9. ~~**What are the real Status values for `cuatro-finance` and `cs-tournament`?** Assumed built-but-not-deployed and Live-on-Vercel respectively (§5.1). Both are rendered-or-not decisions under FR-35 and both are assumptions, not confirmations. *Closes by: Operator confirmation.* Blocks the exact composition of the first public Suite Directory.~~ **Closed 2026-09-02 by Story 2-4**, on the Operator's confirmation: `cuatro-finance` is `In progress` and `cs-tournament` is `Live` at `inclusivcup.vercel.app`, so the first public Suite Directory is the six it was already expected to be (`ops/estate.md`). *(Struck 2026-09-24 by Operator ruling.)* *(Amended 2026-09-25 by Operator ruling 2026-09-24: Vercel is removed from the estate, and `cs-tournament` is `Complete`, still rendered; §5.1 carries the note.)*
